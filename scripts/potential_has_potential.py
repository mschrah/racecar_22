...
class PotentialField:
	...
	self.queue_size = 10
	self.steering_queue = [0]*self.queue_size
	self.d_speed = 0.05
    ...
    def scan_callback(self, msg):
        ...
        command_msg.drive.speed = (self.p_speed * np.sign(total_x_component) * math.sqrt(total_x_component**2 + total_y_component**2)) * 2

	dcomp = self.d_speed * (self.steering_queue[-1] - self.steering_queue[0])
	command_msg.drive.steering_angle -= dcomp
	self.steering_queue.pop(0)
	self.steering_queue.append(command_msg.drive.steering_angle)
